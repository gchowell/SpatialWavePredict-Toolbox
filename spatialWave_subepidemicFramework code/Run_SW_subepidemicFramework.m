function Run_SW_subepidemicFramework(outbreakx_pass)
...
for outbreak1=outbreakx
    ...
    for rank1=1:topmodelsx
        ...
        [Phatss,npatches,onset_thr,typedecline1,curves,bestfit,data1,P0,AICc_best,factor1,d]=fittingModifiedLogisticFunctionPatchMultiple(RMSESx,PS,data1,DT,t_window,M,flag1,numstartpoints,rank1);
  
        cadfilename1=strcat('./output/modifiedLogisticPatch-original-npatchesfixed-',num2str(npatches_fixed),'-onsetfixed-',num2str(onset_fixed),'-smoothing-',num2str(smoothfactor1),'-',datafilename2(1:end-4),'-flag1-',num2str(flag1(1)),'-method-',num2str(method1),'-dist-',num2str(dist1),'-calibrationperiod-',num2str(calibrationperiod1),'-rank-',num2str(rank1),'.mat');
...
clear RMSESx PS P
clear Phatss curves bestfit data1 P0 AICc_best d relativelik_i evidenceRatio
