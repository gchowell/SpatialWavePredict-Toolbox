...
cumulative1=0; % This is a Boolean variable used to indicate if the data file contains cumulative incidence counts (cumulative1=1) or not (cumulative1=0).

outbreakx=1;  % This is an identifier for the spatial area of interest.

caddate1='02-25-2003';  % This is a string variable with the data file time stamp in format: mm-dd-yyyy

cadregion='Singapore'; % This is a string variable indicating the geographic region of the time series contained in the file (Georgia, USA, World, Asia, Africa, etc.)

caddisease='SARS'; % This is a string variable indicating the name of the disease related to the time series data.
...
datevecfirst1=[2003 02 25]; % 3-value date vector that specifies the date corresponding to the first data point in time series data in format: [yyy mm dd]. 

datevecend1=[2003 02 25]; % 3-value date vector that specifies the date of the most recent data file in format: [yyy mm dd].  This data file is used to assess forecast performance.
...
smoothfactor1=1; % This variable indicates the span of the moving average smoothing of the case series (smoothfactor1=1 indicates no smoothing)

calibrationperiod1=70; % This variable indicates the number of most recent data points that will be used to calibrate the model. 
...
numstartpoints=20; % This variable defines the number of different initial guesses for the optimization procedure using Multistart in its search for the globally optimal set of parameters.
...
npatches_fixed=2; % This variable indicates the maximum number of sub-epidemics considered in the epidemic wave model fit.

topmodelsx=2; % This variable specifies the number of best fitting models (based on AICc) that will be generated to derive ensemble models. If npatches_fixed=1 then there is only one model.
